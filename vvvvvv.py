...
pygame.display.set_caption("VVVVVV Hack")
...
class Player:
    ...
    def die(self, exception=False):  # Kill the player
        global noclip
        
        if not noclip or exception:
            sfx_hurt.play()
            self.alive = False
            self.deaths += 1

    def noclipdie(self):  # Don't kill the player
        sfx_hurt.play()
            ...
            for event in events:
                if event.type == pygame.KEYDOWN and self.winTimer == 0:
                    ...
                    if event.key == pygame.K_r:
                        self.die(True)  # Die if you press R
...
def renderHUD():    # Displays time + FPS ingame, plus lots of debug info if F3 is pressed
    ...
    if random.randint(0, 10) == 5:
        fakeFPS = random.randint(59, 61)
    
    if not showFakeFPS:
        fpsCount = smallfont.render(str(int(clock.get_fps())) + " FPS", 1, WHITE)  # Render FPS count
    else:
        fpsCount = smallfont.render(str(int(fakeFPS)) + " FPS", 1, WHITE)  # Render fake FPS count
    ...
    if debug:   # Toggle with F3
        ...
        toggleNoclip = smallfont.render("Noclip (F4) enabled: " + str(noclip), 1, WHITE)
        toggleSpeedhack = smallfont.render("Speedhack (F5) enabled: " + str(speedhack), 1, WHITE)
        toggleFakeFPS = smallfont.render("Fake FPS (F6) enabled: " + str(showFakeFPS), 1, WHITE)
        ...
        screen.blit(toggleNoclip, (10, 70))
        screen.blit(toggleSpeedhack, (10, 90))
        screen.blit(toggleFakeFPS, (10, 110))
        screen.blit(deathCount, (10, 150))
        screen.blit(flipCount, (10, 170))
        screen.blit(starCount, (10, 190))
        screen.blit(roomSpeed, (10, 210))
        screen.blit(roomPos, (10, 230))
        screen.blit(roomData, (10, 250))
    ...
    for event in events:
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if event.key == pygame.K_F4:
                noclip = not noclip   # Toggle noclip upon pressing F4
            if event.key == pygame.K_F5:
                speedhack = not speedhack   # Toggle speedhack upon pressing F5
            if event.key == pygame.K_F6:
                showFakeFPS = not showFakeFPS   # Toggle speedhack upon pressing F5
